...
export interface FlashcardsSortOption {
  field: "created_at" | "updated_at";
  order: "asc" | "desc";
}

export interface FormErrorViewModel {
  message: string;
  code?: string;
  field?: "front" | "back";
}
...
export const useFlashcards = () => {
  ...
  const setSource = useCallback((source?: FlashcardSource) => updateQuery({ source, page: 1 }), [updateQuery]);

  const setSort = useCallback(
    (sort: FlashcardsSortOption) => updateQuery({ sort: sort.field, order: sort.order, page: 1 }),
    [updateQuery]
  );
  ...
  const updateFlashcard = useCallback(async (id: number, payload: UpdateFlashcardCommand) => {
    setError(undefined);
    setItems((current) => current.map((item) => (item.id === id ? { ...item, isUpdating: true } : item)));
    try {
      ...
      const data = (await response.json()) as FlashcardDto;
      setItems((current) => current.map((item) => (item.id === id ? mapFlashcard(data) : item)));
      return true;
    } catch {
      setError({ message: "Brak połączenia z serwerem. Spróbuj ponownie." });
      return false;
    } finally {
      setItems((current) => current.map((item) => (item.id === id ? { ...item, isUpdating: false } : item)));
    }
  }, []);
  ...
  const deleteFlashcard = useCallback(
    async (id: number) => {
      ...
      setItems((current) => current.map((item) => (item.id === id ? { ...item, isDeleting: true } : item)));
