...
type AuthResult = { ok: true; accessToken: string; userId: string } | { ok: false; response: Response };
...
const setSupabaseSession = async (supabase: SupabaseClient, accessToken: string, refreshToken: string | null) => {
  if (refreshToken) {
